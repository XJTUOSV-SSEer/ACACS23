...
static sgx_status_t SGX_CDECL CryptoEnclave_ocall_send_Proof_res(void* pms)
{
	...
	ocall_send_Proof_res(ms->ms_Proof_res, ms->ms_len, ms->ms_pi_w, ms->ms_pi_u, ms->ms_w, ms->ms_acc);
...
sgx_status_t ecall_get_Acc(sgx_enclave_id_t eid, const char* acc)
{
	sgx_status_t status;
	ms_ecall_get_Acc_t ms;
	ms.ms_acc = acc;
	ms.ms_acc_len = acc ? strlen(acc) + 1 : 0;
