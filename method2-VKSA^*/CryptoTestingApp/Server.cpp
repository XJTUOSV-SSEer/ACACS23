...
serverSearchRes Server::getSearch_token(search_token st){
  ...
  serverSearchRes res; //返回结果
  res.e_u = e_u; //ID_u的密文搜索结果
  res.Sum_e_w = sum_e_w; //w的密文搜索结果
  res.w = charToString(st.w,st.w_len); //w
  res.id_u = charToString(st.id_u,st.id_u_len); //ID_u
...
void Server::getProduct(std::string product){
  this->product = product;
}

void Server::updateProduct(std::string old_h,std::string new_h){
    product = divide(product,old_h);
    product = multiply(product,new_h);
}

std::string Server::sendProduct(){
  return product;
}
